 ...
 *
 * <p>A Polygon is defined by a series of linear rings, where the first ring is the exterior of the polygon,
 ...
 * </p>
 *
 * <p>This class is designed to be used with frameworks that serialize and deserialize GeoJSON objects, such as
 ...
 * }</pre>
 *
 * <p>The `validate()` method can be called to check whether the polygon coordinates are valid:
 * </p>
 * <pre>{@code
...
public class PolygonCoordinates implements Validatable, Serializable {
     ...
     *
     * <p>This constructor is required for certain frameworks (e.g., ORM frameworks) and serialization mechanisms
     ...
     *
     * <p>The first linear ring is assigned to the exterior of the polygon,
     ...
     *
     * <p>This method constructs a {@link PolygonCoordinates} object using a varargs parameter, where each provided
     ...
     *
     * <p>This method constructs a {@link PolygonCoordinates} object using the given list of linear rings,
     ...
     *
     * <p>This method constructs a {@link PolygonCoordinates} object using the given exterior and holes,
    ...
    private static Set<ValidationError> validateLinerRing(List<Position> linearRing) {
        ...
        if (CollectionUtils.size(linearRing) < 4) {
            errors.add(ValidationError.of("coordinates", "Ring '%s' must contain at least four positions.".formatted(linearRing),
                    "coordinates.ring.length.invalid"));
        }
        if (CollectionUtils.isNotEmpty(linearRing) && !linearRing.getFirst().equals(linearRing.getLast())) {
            errors.add(ValidationError.of("coordinates", "Ring '%s', first and last position must be the same.".formatted(linearRing),
                    "coordinates.ring.circle.invalid"));
        }
     ...
     *
     * <p>This method checks that the exterior and holes are valid according to the rules for polygons:
